...
export default function OrderTrackingClient() {
  const routeId = useRouteId("/account/orders/");
  const orderId = routeId.status === "resolved" ? routeId.id : null;
    ...
    const fetchOrder = async () => {
      try {
        ...
        const found = response.success
          ? (response.data.orders ?? []).find((o) => o._id === orderId)
          : null;
        setOrder(found ?? null);
      } catch (error) {
  ...
  const handleCancelOrder = async () => {
    // Unreachable in practice: the button only renders once an order loaded,
    // which requires a resolved id. TypeScript can't see that.
    if (!orderId) return;
